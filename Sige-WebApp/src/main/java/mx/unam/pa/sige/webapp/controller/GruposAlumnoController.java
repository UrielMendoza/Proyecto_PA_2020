...
public class GruposAlumnoController {
	@Autowired
    ...
    public ModelAndView listar(@ModelAttribute("usuarioFirmado") Alumnos alumno){
		...
		List<MateriasHorarios> alumnoGrupos = alumnosServicio.listarGrupoAlumno(alumno.getGrupo().getIdGrupo());	
        
		view.addObject("gruposAlumno", alumnoGrupos);
		view.setViewName("grupos-alumno");
		return view;
    }
